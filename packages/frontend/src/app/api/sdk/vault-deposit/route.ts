...
//   signer: await embeddedWallet.account(), // Embedded wallet как подписант
      // });
      const smartWallet = await myceliumService.getSDK()!.wallet.getSmartWalletWithEmbeddedSigner({ walletId });
      console.log(" ====== Wallet: ====== ", smartWallet);
      

    const beefyProtocol = new BeefyProtocol(myceliumService.getSDK()!.chainManager);
    const result = await beefyProtocol.deposit(amount, vaultInfo, walletAddress, chainId, smartWallet);
